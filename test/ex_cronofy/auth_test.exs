defmodule ExCronofy.AuthTest do
  ...
  describe "request_authorization_url/3" do
    test "returns authorization uri with correct query params" do
      ...
      resultant_uri = ExCronofy.Auth.request_authorization_url(
        required_query_params.scope, 
        required_query_params.redirect_uri,
        optional_params
      )

      resultant_uri = resultant_uri
                      |> URI.parse()

      assert "#{resultant_uri.scheme}://#{resultant_uri.host}#{resultant_uri.path}" == "https://app.cronofy.com/oauth/authorize"

      expected_query_params = Map.merge(required_query_params, optional_params)
